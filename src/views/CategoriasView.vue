    ...
    <div class="container py-5 px-2 bg-primary">
        <div class="row py-5 px-4">
            <div class="col-sm-6 text-center text-md-left">
                <h1 class="mb-3 mb-md-0 text-white text-uppercase font-weight-bold">Categorias</h1>
            </div>
    ...
    <div class="container py-5 px-2 bg-white">
        <div class="row px-4">
            <div v-for="item in categorias" :key="item.id" class="col-md-6">
                <div @click="filtrar(item.nombre)" class="card text-white bg-dark m-1">
                    <img :src="item.imagen" class="card-img" alt="..." style="width:100%; max-height:220px">
                    <div class="card-img-overlay text-center">
                        <br> <br>
                        <h2 class="card-title text-white p-3 shadow-lg"> <b>{{ item.nombre }}</b></h2>
                        <p class="card-text text-white"><i class="fas fa-list">{{ item.cant_posts }} Posts</i></p>
                    </div>
                </div>
            </div>
        </div>
